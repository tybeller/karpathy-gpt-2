...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # nh is the "number of heads", hs is the "head size", and C is "number of channels" where C = nh * hs
        # e.g. in GPT-2 (124M), n_head=12, hs=64, so nh*hs=C=768 channels in the Transformer
        qkv = self.c_attn(x) # q is "query", k is "key", v is "value"
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        if targets is not None:
            loss = F.cross_entropy(logits.view(-1, logits.size(-1)), targets.view(-1)) # cross entropy doesn't like multidimensional inputs so we flatten from 3d to 2d
        return logits, loss
...
if torch.cuda.is_available():
    device = "cuda"
elif hasattr(torch.backends, "mps") and torch.backens.mps.is_available():
    device = "mps"
...
# generate! righ now x is (B, T) where B = 5 and T = 8
# set the seed to 42
